"""Abstract base classes for models (taggers, tokenizers, sentiment analyzers, etc.)
...
class BaseTokenizer(nltk.tokenize.api.TokenizerI, metaclass=ABCMeta):
    ...
    that returns a list of tokens as strings.
    """
...
class BaseTagger(metaclass=ABCMeta):
    """Abstract tagger class from which all taggers inherit from.
    All descendants must implement a ``tag()`` method.
    """
...
class BaseSentimentAnalyzer(metaclass=ABCMeta):
    ...
    def analyze(self, text):
        """Return the result of analysis. Typically returns either a
